from src.main.java.training.katas.WhereMyAnagramsAt.where_my_anagrams_at \
    import anagrams
...
def test_where_my_anagrams_at(word: str,
                              words: List[str], expected_result: List[str]):
    assert expected_result == anagrams(word, words)


'''
Test.assert_equals(anagrams('abba',
['aabb', 'abcd', 'bbaa', 'dada']), ['aabb', 'bbaa'])
Test.assert_equals(anagrams('racer',
